...
\begin{itemize}
    ...
    \begin{enumerate}
        \item Mixed-facility baseline: all five facilities pooled and split with a stratified train/val/test split (Section~\ref{sec:exp_mixed}).
        \item Leave-one-facility-out evaluation: the current main experiment, each facility held out as an unseen test facility in turn (Section~\ref{sec:exp_lofo}). Used to estimate real-world generalisation to a facility the model has never seen.
        \item Binary (OK / Not-OK) approach: collapsing the multi-class labels into a two-class problem (Section~\ref{sec:exp_binary}).
...
\node[stage, below=of split] (aug) {Resize, normalise, and (train only) augment each image (Section~\ref{sec:exp_augmentation})};
\node[stage, below=of aug] (model) {Pretrained backbone with a tuned classifier head (Sections~\ref{sec:exp_architectures} and~\ref{sec:exp_hyperparams})};
...
Figure~\ref{fig:experimental_pipeline} summarises this pipeline end to end. It starts from the raw, labelled satellite imagery described in Chapter~\ref{chap:systemmethods}, which \texttt{prep.py} crops into individual, per-class images and which is then split by facility so that no image from the held-out test facility ever appears during training. Each image is resized, normalised, and, for the training set only, augmented before being fed to the network (Section~\ref{sec:exp_augmentation}). The network itself is one of the seven architectures in Section~\ref{sec:exp_architectures}, each with a classifier head sized by its own tuned hyperparameters (Section~\ref{sec:exp_hyperparams}), trained using the loop described in Section~\ref{sec:exp_training}. The outcome of a single (architecture, held-out facility) run, its test metrics, confusion matrix, training history and per-sample predictions, is pickled to its own \texttt{results\_*.pkl} file (Section~\ref{sec:exp_lofo}) rather than kept only in memory, so that the comparison notebook can later load every run's results and build the cross-architecture, cross-facility comparisons used in the Results chapter without needing to retrain anything.
