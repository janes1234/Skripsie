...
The dataset used in this project is completely new, and wass prepared shorty before the project started by annotators under the supervision of Prof F. de Wet. Inspection of the ata in the early phasesof experimental design however identified the occational occurence of labelling inconsistencies or errors. To improve label quality, a relabeling notebook was built using Jupyter widgets to review the clarifier dataset in full and correct labels where
necessary. Every clarifier image was reviewed manually, rather than considering only images the classifier identified incorrectly which could potentially introduce bias. For each image, the notebook displays the image alongside its current label and a row of per-class buttons; clicking a button records the corrected label without requiring any manual file editing. Corrections are written to a separate \texttt{relabel/} working copy of the annotation files rather than overwriting the originals, so the original VIA2 labels remain intact and every correction can be traced back to the annotations as originally supplied.

Figure \ref{fig:relabel_example} shows a case, where an image originally labelled ``Scum'' was corrected to ``Empty'' after review. The dark
region was identified as shadow a rather than surface scum. Figure \ref{fig:relabel_process_example} shows the review interface itself, with the original label displayed in yellow next to the image and the correction buttons below it.
...
Table \ref{tab:relabel_corrections} summarises every correction applied during the relabeling process, broken down by facility/unit and by class. For each class, the two values given are the number of clarifier regions that were corrected \emph{away from} that label (``out'') and the number that were newly assigned that label as a result of review (``in''). A class with a high ``in'' count and low ``out'' count indicates that reviewers frequently reassigned other classes to it, most commonly because the region had originally been over-labelled and was in fact empty or unaffected; the ``Empty'' class shows exactly this pattern, gaining 113 corrections while losing only 5. In total, 293 clarifier region labels were changed during manual review.

\begin{table}[htbp]
    \centering
    \caption{Clarifier label corrections applied during relabeling, by facility/unit and class. Each cell gives the number of regions corrected away from that class (out) and into that class (in).}
    \label{tab:relabel_corrections}
    \begin{tabular}{l cccc r}
        \toprule
        \textbf{Facility} & \textbf{Dysfunctional} & \textbf{Empty} & \textbf{Functional} & \textbf{Scum} & \textbf{Total} \\
        & \scriptsize{(out/in)} & \scriptsize{(out/in)} & \scriptsize{(out/in)} & \scriptsize{(out/in)} & \\
        \midrule
        Atlantis         & 0/0   & 0/1   & 2/1   & 1/1   & 3   \\
        Cape Flats 1     & 6/2   & 5/19  & 18/6  & 11/13 & 40  \\
        Cape Flats 2     & 10/0  & 0/15  & 11/2  & 3/7   & 24  \\
        Cape Flats 3     & 10/4  & 0/28  & 15/0  & 11/4  & 36  \\
        Cape Flats 4     & 25/2  & 0/44  & 7/1   & 19/4  & 51  \\
        Waterval Top     & 71/0  & 0/0   & 9/3   & 0/77  & 80  \\
        Waterval Bottom  & 0/0   & 0/0   & 0/2   & 2/0   & 2   \\
        Fisantekraal     & 2/1   & 0/1   & 1/5   & 6/2   & 9   \\
        Northern Works   & 2/15  & 0/5   & 35/2  & 11/26 & 48  \\
        \midrule
        \textbf{TOTAL}   & \textbf{126/24} & \textbf{5/113} & \textbf{98/22} & \textbf{64/134} & \textbf{293} \\
        \bottomrule
    \end{tabular}
\end{table}
